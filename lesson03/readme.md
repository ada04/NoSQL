...
Сравнивались два запроса по коллекции `calendar`.
В первом случае выполнялась фильтрация по полю `available` (значения "f" or "t") во втором по полю `date`.
В первом варианте запрос без индекса оказался выгоднее (выполнился за  сек против  сек с индексом)
Во втором варианте запрос с индексом оказался выгоднее (выполнился за 58 сек против 202 сек без индекса)
