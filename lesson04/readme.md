...
Урок 04 - Кластерные возможности mongodb
...
Изменяем "слушатель" на все IP, не только локальный
...
Также нужно включить авторизацию (ниже в тексте создаётся пользователь)
Если это пропустить, то открытая во вне БД может быть украдена в течении менее часа, как и произошло у меня с тестовой БД :)
